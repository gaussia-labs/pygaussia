...
| `ground_truth_agentic` | `dict \| None` | `{}` | Expected tool usage and declared tool scope (for the Agentic metric). Recognized keys: `expected_tools`, `allowed_tools`, `tool_sequence_matters` — anything else is rejected, so a misspelling cannot silently disable a check. Free-form metadata belongs in `agentic` |
