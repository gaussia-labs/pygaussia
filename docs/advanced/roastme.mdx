...
[`skills/`](https://github.com/gaussia-labs/pygaussia/tree/main/skills) beside the code they describe,
...
[`roastme_quickstart.ipynb`](https://github.com/gaussia-labs/pygaussia/tree/main/examples/roastme/jupyter),
...
`Articulo_25`. That is the shape of a corpus of numbered clauses. On scraped prose it recognises the
wrong things — phone numbers, document filenames, footer anchors — and the engine then generates a
probe per false positive **with no error**, because a junk boundary is indistinguishable from a
legitimate one at that layer. On one real bank corpus it returned 210 false positives across twelve
documents.

```python
from gaussia.generators.roastme.probes.mentions import CompoundTokenExtractor

print(sorted(CompoundTokenExtractor().extract(documents))[:30])   # run this first
```

If that comes back wrong, inject a reading of your own — the default stays the compound-identifier
one, so a corpus that worked before still works:
...
shipped engines, not part of the specification you implement against, so the ten interfaces stay ten.
`EnumerationProbeEngine` takes no extractor — its boundary comes from your enumerator instead, which
is the other answer to the same problem and the stronger one when you can enumerate.
...
`FindingsReporter` is an optional post-processing step over a completed Profiler run. It sends only
positive-violation or ungraded probes, the contract, grader context, corpus and transport audit to an
injected structured-output chat model. The model cannot modify grades or rates. Its validated output
can be persisted as JSON and rendered deterministically as Markdown.

```python
from pathlib import Path

from gaussia.generators.roastme import FindingsReporter, render_findings_markdown
...
findings = reporter.interpret(
    ...
    result=result,
    documents=documents,
    grader_context={"method": "your-grader-method"},
    transport_audit=transport_audit,
)

Path("findings.json").write_text(findings.model_dump_json(indent=2))
Path("FINDINGS.md").write_text(render_findings_markdown(findings))
```

The report separates the finding kind from the grader assessment. A non-scoreable control can expose
a real unsupported claim while also carrying `grader_assessment="taxonomy_mismatch"` because the
configured principle does not cover that claim. Probe and principle references are validated before
the report is returned.

<Warning>
  A findings report is an interpretation, not another metric. If the reviewer uses the same model as
  the target, `same_model_as_target` records that the review is correlated and not independent.
  Persist the structured JSON, keep the original Dataset, and review consequential findings manually.
</Warning>
