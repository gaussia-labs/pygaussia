...
| `evalhub` | EvalHub BYOF provider adapter | eval-hub-sdk, requests |
...
<CodeGroup>
  ...
  ```bash EvalHub Provider
  # Install the EvalHub provider adapter
...
When running the EvalHub provider adapter as a process, configure the judge with a LangChain-compatible connector. You can either point to a connector class directly:

```bash
export GAUSSIA_JUDGE_CONNECTOR_CLASS="your_package.YourChatModel"
export GAUSSIA_JUDGE_CONNECTOR_KWARGS_JSON='{"model": "your-judge-model"}'
```

Or use LangChain's provider registry:

```bash
export GAUSSIA_JUDGE_MODEL="your-judge-model"
export GAUSSIA_JUDGE_MODEL_PROVIDER="your-provider"
```
